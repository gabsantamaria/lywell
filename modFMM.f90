module modFMM
    ...
    integer (kind = il) :: n_esparcida_FMM
    integer (kind = il), allocatable, dimension(:) :: ia_FMM,ja_FMM
...
contains
    ...
    subroutine inicializar_FMM (arg_e_centro, arg_num_e, arg_e_t, arg_e_po, arg_e_long, arg_t_area, arg_t_baric, arg_t_baric_sub, arg_p_coord, arg_t_p, arg_e_p, arg_num_p, arg_num_t)
        !dummy
        ...
        print*, 'Se tienen ' // inttostr(n_multipolos) // ' multipolos y ' // inttostr(n_kappas) // ' muestras sobre la esfera unitaria'
        print*, 'Hecho'
    ...
    subroutine geomFMM(lCuboMayor, esquinaX, esquinaY, esquinaZ)
        ...
        n_cubos_eje = ceiling(lCuboMayor/(criterio_ladoW))
    ...
    subroutine llenar_funcRAD(maxPrecision)
        ...
        real (kind = dp), dimension(3) :: baricentroplus, vopuestoplus, rhoplus, baricentrominus, vopuestominus, rhominus, vkappa, centro
