...
contains
    ...
    subroutine getcommand_console(comm, val)
        !dummy
        character (len = 20), intent(out) :: comm, val
        !
        !local
    ...
    !*  (matriz) de tipo real y con formato 'cuadrado' en el archivo
    !*  ************************************************************
    subroutine guardarVectorR(vector, nombre_archivo)
        !dummy
    ...
    !*	(matriz) de tipo real y con formato 'cuadrado' en el archivo
    !*  ************************************************************
    subroutine guardarMatrizR(matriz, nombre_archivo)
        !dummy
    ...
    subroutine frecuenciaMaxMallado(t_area, e_long, num_t, num_e)
        ...
        print*, 'Lambda minimo que puede ser usado para un mallado perfecto (criterio longitud RWG): ' // realtostr(factor*lMax)
        print*, 'Lambda minimo que puede ser usado para un mallado 95.44% correcto (criterio longitud RWG): ' // realtostr(factor*(lProm+2*lDesv))
        ...
        print*, 'Lambda minimo que puede ser usado para un mallado perfecto (criterio area parches): ' // realtostr(factor*sqrt(aMax))
        print*, 'Lambda minimo que puede ser usado para un mallado 95.44% correcto (criterio parches): ' // realtostr(factor*sqrt(aProm+2.*aDesv))



        print*, 'Lambda minimo que puede ser usado para un mallado correcto (criterio promedio superficie total): ' // realtostr(factor*sqrt(aTotal/num_e))
