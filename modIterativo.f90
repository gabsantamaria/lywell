module modIterativo
    ...
    implicit none
    private
    public :: CGS, csrmsr, ilut, msrcsr, csrcoo, coocsr, ilu0, csrmsrC, msrcsrC, csrcooC, coocsrC, ilu0C, lusol0C, CGSold, ilutC, amuxC

contains



    subroutine CGS(func_MatxVec, b, guess, errmax, n, x, mensaje, prec_alu, prec_jlu, prec_ju)
        !dummy
        !complex (kind = dp), intent(in), dimension(n,n) :: Z

        complex (kind = dp), allocatable, dimension(:) :: copiab
!=|        complex (kind = dp), intent(in), dimension(n) :: guess, b
        complex (kind = dp), intent(inout), dimension(n) :: guess, b
        ...
        integer (kind = il), optional, intent(in), dimension(:) :: prec_ju

        !
        !local
        complex (kind = dp),  dimension(n) :: r, R_, u, p, P_, V_, q, U_, Q_ !_ = gorrito
        complex (kind = dp) :: beta, alfa
        complex (kind = dp), dimension(2) :: rho !rho(1) = i-1; rho(2) = i-2
        integer (kind = il) :: i
        logical :: prec
        !character (len = 30) ttt
        !
        ...
        if (prec) then
          allocate(copiab(n))
          copiab = b
          call lusol0C(n, copiab, b, prec_alu, prec_jlu, prec_ju)
          deallocate(copiab)
        end if
        ...
        do
            ...
            !rho(1) = Nproducto_escalarC(r,R_,n)
            rho(1) = sum(r*R_)
            !print*, sqrt(rho(1))-NmoduloC(r,n)
            ...
            call lusol0C(n, func_MatxVec(P_), V_, prec_alu, prec_jlu, prec_ju)
            ...
            call lusol0C(n, func_MatxVec(U_), Q_, prec_alu, prec_jlu, prec_ju)
            r = r -(alfa*Q_)
            ...
            if (present(mensaje)) then
                ...
                !Write(strprint, '(f10.4)') NmoduloC(r,n)
                !Write(strprint2, '(f10.4)') NmoduloC(b,n)
                !Write(strprint3, '(I5)') i
                !print*, 'Iteracion: ' // strprint3 // ' residuo ' // strprint // ' de ' // strprint2
                print*, 'Iteracion: ' // inttostr(i) // ' residuo ' , sqrt(real(dot_product(r,r))) , ' de ' , sqrt(real(dot_product(b,b)))
            ...
            !if (NmoduloC(r,n) < (errmax*NmoduloC(b,n)) ) then
            if (  sqrt(real(dot_product(r,r))) < errmax*sqrt(real(dot_product(b,b))) ) then
                if (present(mensaje)) then
                    !Write(strprint, '(f10.8)') NmoduloC(r,n)/NmoduloC(b,n)
                    !Write(strprint2, '(f10.8)') NmoduloC(matmul(Z,x)-b,n)/NmoduloC(b,n)

                    !print*, 'El sistema ha convergido con error: ' // strprint
                    print*, 'El sistema ha convergido con error: ' , sqrt(real(dot_product(r,r)/dot_product(b,b)))
                    !print*, 'El error relativo resulto: ' // strprint2
    ...
    subroutine CGSold_sin_optional(func_MatxVec, b, guess, errmax, n, x, mensaje, prec_alu, prec_jlu, prec_ju)
        !dummy
        !complex (kind = dp), intent(in), dimension(n,n) :: Z
...
!        complex (kind = dp), intent(inout), dimension(n) :: guess, b
        ...
        real (kind = dp),  intent(in) :: errmax
        integer (kind = il),  intent(in) :: n
        integer (kind = il),  intent(in) :: mensaje
        complex (kind = dp_prec),  intent(in), dimension(:) :: prec_alu
        integer (kind = il),  intent(in), dimension(:) :: prec_jlu
        integer (kind = il),  intent(in), dimension(:) :: prec_ju
...
!        end if
        ...
        do
            ...
            if (prec == .false.) then
              ...
              print*, 'Aplicando precondicionador 1'
              call msjtiempo()
              call lusol0C(n, p, P_, prec_alu, prec_jlu, prec_ju)
              call msjtiempo()
              print*, 'Hecho precondicionador 1'
            end if
            ...
            if (prec == .false.) then
              ...
              print*, 'Aplicando precondicionador 2'
              call msjtiempo()
              call lusol0C(n, u+q, U_, prec_alu, prec_jlu, prec_ju)
              call msjtiempo()
              print*, 'Hecho precondicionador 2'
            end if
...
!        end if
        ...
        do
            ...
            if (prec == .false.) then
              ...
              !call printInLine('Aplicando precondicionador 1')
              !call msjtiempo()
              call lusol0C(n, p, P_, prec_alu, prec_jlu, prec_ju)
              !call msjtiempo()
              !call printInLine('Hecho precondicionador 1       ')
            end if
            ...
            if (prec == .false.) then
              ...
              !call printInLine('Aplicando precondicionador 2')
              !call msjtiempo()
              call lusol0C(n, u+q, U_, prec_alu, prec_jlu, prec_ju)
              !call msjtiempo()
              !call printInLine('Hecho precondicionador 2       ')
            end if
            ...
            if (present(mensaje)) then
                ...
                !print*, 'Iteracion: ' // inttostr(i) // ' residuo ' , sqrt(real(dot_product(r,r))) , ' de ' , sqrt(real(dot_product(b,b)))
                !call printInLine(gettiempo() // '                   ')
                print*, 'Iterac: ' // inttostr(i) // ' residuo ' //  trim(realtostrE2(resid,5)) // ' de ' // trim(realtostrE2(residde,5)) // ' - ' // trim(gettiempo())
            ...
            if (  sqrt(real(dot_product(r,r))) < errmax*sqrt(real(dot_product(b,b))) ) then
                if (present(mensaje)) then
                    ...
                    !print*, 'El sistema ha convergido con error: ' // strprint
                    !print*, 'El sistema ha convergido con error: ' , sqrt(real(dot_product(r,r)/dot_product(b,b)))
                    call savetolog('El sistema ha convergido con error: ' // realtostrE(sqrt(real(dot_product(r,r)/dot_product(b,b)))),.true.)
                    !print*, 'El error relativo resulto: ' // strprint2
